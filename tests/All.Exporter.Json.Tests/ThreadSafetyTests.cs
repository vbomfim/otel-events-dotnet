...
public sealed class ThreadSafetyTests
{
    ...
    public void Export_LockTimeout_DropsBatchAndReturnsSuccess()
    {
        ...
        var options = new AllJsonExporterOptions { LockTimeout = TimeSpan.FromMilliseconds(1) };
        var stream = new MemoryStream();
        var exporter = new AllJsonExporter(options, stream);
        ...
        var lockField = typeof(AllJsonExporter)
    ...
    public void Export_IOException_ReturnsFailure()
    {
        ...
        var exporter = new AllJsonExporter(options, failingStream);
    ...
    {
        var stream = new MemoryStream();
        var options = new AllJsonExporterOptions { LockTimeout = TimeSpan.FromSeconds(5) };
        var exporter = new AllJsonExporter(options, stream);
