...
public sealed class MetadataTests
{
    ...
    {
        using var harness = new TestExporterHarness(new AllJsonExporterOptions { SchemaVersion = "2.0.0" });
    ...
    {
        using var harness = new TestExporterHarness(new AllJsonExporterOptions { EmitHostInfo = false });
        var lr = TestExporterHarness.CreateLogRecord(eventName: "test.event");
    ...
    {
        using var harness = new TestExporterHarness(new AllJsonExporterOptions { EmitHostInfo = true });
        var lr = TestExporterHarness.CreateLogRecord(eventName: "test.event");
