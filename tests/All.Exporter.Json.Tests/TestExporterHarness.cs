...
/// <summary>
/// Helper to create an <see cref="AllJsonExporter"/> that writes to an in-memory stream
/// and captures the JSONL output for assertions.
/// </summary>
internal sealed class TestExporterHarness : IDisposable
{
    private readonly MemoryStream _stream;
    private readonly AllJsonExporter _exporter;

    public TestExporterHarness(AllJsonExporterOptions? options = null)
    {
        _stream = new MemoryStream();
        options ??= new AllJsonExporterOptions();
        _exporter = new AllJsonExporter(options, _stream);
    }

    public AllJsonExporter Exporter => _exporter;
