using All.Causality;

namespace All.Causality.Tests;

/// <summary>
/// Tests for AllCausalityContext — AsyncLocal-based parent event ID management.
/// Validates scope nesting, async propagation, and thread isolation.
/// </summary>
public class AllCausalityContextTests
{
    [Fact]
    public void CurrentParentEventId_IsNullByDefault()
    {
        // Assert — no scope active
        Assert.Null(AllCausalityContext.CurrentParentEventId);
    }

    [Fact]
    public void SetParent_SetsCurrentParentEventId()
    {
        // Arrange
        var parentId = "evt_test-parent-id";

        // Act
        using var scope = AllCausalityContext.SetParent(parentId);

        // Assert
        Assert.Equal(parentId, AllCausalityContext.CurrentParentEventId);
    }

    [Fact]
    public void SetParent_RestoresPreviousValueOnDispose()
    {
        // Arrange — no parent initially
        Assert.Null(AllCausalityContext.CurrentParentEventId);

        // Act
        using (AllCausalityContext.SetParent("evt_parent-1"))
        {
            Assert.Equal("evt_parent-1", AllCausalityContext.CurrentParentEventId);
        }

        // Assert — restored to null
        Assert.Null(AllCausalityContext.CurrentParentEventId);
    }

    [Fact]
    public void SetParent_SupportsNesting_RestoresEachLevel()
    {
        // Arrange
        Assert.Null(AllCausalityContext.CurrentParentEventId);

        // Act & Assert — three levels of nesting
        using (AllCausalityContext.SetParent("evt_grandparent"))
        {
            Assert.Equal("evt_grandparent", AllCausalityContext.CurrentParentEventId);

            using (AllCausalityContext.SetParent("evt_parent"))
            {
                Assert.Equal("evt_parent", AllCausalityContext.CurrentParentEventId);

                using (AllCausalityContext.SetParent("evt_child"))
                {
                    Assert.Equal("evt_child", AllCausalityContext.CurrentParentEventId);
                }

                // Restored to parent
                Assert.Equal("evt_parent", AllCausalityContext.CurrentParentEventId);
            }

            // Restored to grandparent
            Assert.Equal("evt_grandparent", AllCausalityContext.CurrentParentEventId);
        }

        // Restored to null
        Assert.Null(AllCausalityContext.CurrentParentEventId);
    }

    [Fact]
    public async Task SetParent_FlowsAcrossAsyncAwait()
    {
        // Arrange
        var parentId = "evt_async-parent";

        // Act
        using var scope = AllCausalityContext.SetParent(parentId);

        // Assert — flows across async boundaries
        await Task.Yield();
        Assert.Equal(parentId, AllCausalityContext.CurrentParentEventId);

        await Task.Delay(1);
        Assert.Equal(parentId, AllCausalityContext.CurrentParentEventId);

        var valueInTask = await Task.Run(() => AllCausalityContext.CurrentParentEventId);
        Assert.Equal(parentId, valueInTask);
    }

    [Fact]
    public async Task SetParent_FlowsAcrossAsyncAwait_WithNesting()
    {
        using (AllCausalityContext.SetParent("evt_outer"))
        {
            Assert.Equal("evt_outer", AllCausalityContext.CurrentParentEventId);

            await Task.Yield();
            Assert.Equal("evt_outer", AllCausalityContext.CurrentParentEventId);

            using (AllCausalityContext.SetParent("evt_inner"))
            {
                await Task.Yield();
                Assert.Equal("evt_inner", AllCausalityContext.CurrentParentEventId);
            }

            await Task.Yield();
            Assert.Equal("evt_outer", AllCausalityContext.CurrentParentEventId);
        }

        Assert.Null(AllCausalityContext.CurrentParentEventId);
    }

    [Fact]
    public async Task SetParent_IsolatesBetweenConcurrentTasks()
    {
        // Arrange — two concurrent tasks with different parent IDs
        var barrier = new Barrier(2);

        var task1 = Task.Run(() =>
        {
            using var scope = AllCausalityContext.SetParent("evt_task1-parent");
            barrier.SignalAndWait(); // synchronize start
            Thread.Sleep(50); // overlap execution
            return AllCausalityContext.CurrentParentEventId;
        });

        var task2 = Task.Run(() =>
        {
            using var scope = AllCausalityContext.SetParent("evt_task2-parent");
            barrier.SignalAndWait(); // synchronize start
            Thread.Sleep(50); // overlap execution
            return AllCausalityContext.CurrentParentEventId;
        });

        // Act
        var results = await Task.WhenAll(task1, task2);

        // Assert — each task saw its own parent, not the other's
        Assert.Equal("evt_task1-parent", results[0]);
        Assert.Equal("evt_task2-parent", results[1]);
    }

    [Fact]
    public void SetParent_ThrowsOnNullParentEventId()
    {
        // Assert — null parent ID should throw
        Assert.Throws<ArgumentNullException>(() => AllCausalityContext.SetParent(null!));
    }

    [Fact]
    public void SetParent_ThrowsOnEmptyParentEventId()
    {
        // Assert — empty parent ID should throw
        Assert.Throws<ArgumentException>(() => AllCausalityContext.SetParent(""));
    }

    [Fact]
    public void CurrentParentEventId_CanBeSetDirectly()
    {
        // Arrange
        try
        {
            // Act
            AllCausalityContext.CurrentParentEventId = "evt_direct-set";
            Assert.Equal("evt_direct-set", AllCausalityContext.CurrentParentEventId);
        }
        finally
        {
            // Cleanup
            AllCausalityContext.CurrentParentEventId = null;
        }
    }
}
