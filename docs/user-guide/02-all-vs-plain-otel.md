# Chapter 2 — ALL vs Plain OTEL: Side-by-Side Comparison
...
### ALL JSON output (AllJsonExporter)
...
| **Causal linking** | Not available | `AllCausalityProcessor` adds `eventId` / `parentEventId` |
| **JSON output** | OTEL Console exporter (verbose, multi-line) | `AllJsonExporter` (compact, single-line JSONL) |
...
builder.Services.AddOpenTelemetry()
    ...
    .WithLogging(logging =>
    {
        // ─── ALL extensions (add these) ────────────────────────────────
        logging.AddProcessor<AllCausalityProcessor>();  // eventId/parentEventId
        logging.AddAllJsonExporter(options =>            // AI-optimized JSONL
        {
...
Your existing OTEL exporters, traces, and resource configuration stay exactly the same. The `LogRecord`s generated by ALL are standard OTEL `LogRecord`s — every exporter in your pipeline sees them as native records.
