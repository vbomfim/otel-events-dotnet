...
This document describes the trust boundaries, threat vectors, and mitigations for otel-events components.
...
- **AsyncLocal trust**: `OtelEventsCausalityContext` uses `AsyncLocal<T>` — any code in the async flow can set `parentEventId`. Cross-process causality requires OTEL trace context propagation, not `AsyncLocal`.
...
- Covers **all** attribute values, not just non-otel-events LogRecords
