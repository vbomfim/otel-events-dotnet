...
public sealed class OtelEventsJsonExporter : BaseExporter<LogRecord>
{
    ...
    /// <summary>
    /// Initializes a new instance of <see cref="OtelEventsJsonExporter"/> writing to the given stream.
    /// </summary>
    internal OtelEventsJsonExporter(OtelEventsJsonExporterOptions options, Stream output)
    {
    ...
    /// <summary>
    /// Initializes a new instance of <see cref="OtelEventsJsonExporter"/> using the configured output target.
    /// </summary>
    public OtelEventsJsonExporter(OtelEventsJsonExporterOptions options)
    ...
    internal static void WriteExceptionObject(
        ...
        OtelEventsEnvironmentProfile profile,
        int depth)
    {
    ...
    private static void WriteStackTrace(Utf8JsonWriter writer, Exception exception, OtelEventsEnvironmentProfile profile)
    {
        var stackTrace = new StackTrace(exception, fNeedFileInfo: profile == OtelEventsEnvironmentProfile.Development);
        ...
        foreach (var frame in frames)
        {
            ...
            if (profile == OtelEventsEnvironmentProfile.Development)
            {
    ...
    private static Stream ResolveOutputStream(OtelEventsJsonExporterOptions options) =>
        options.Output switch
        {
            OtelEventsJsonOutput.Stdout => Console.OpenStandardOutput(),
            OtelEventsJsonOutput.Stderr => Console.OpenStandardError(),
            OtelEventsJsonOutput.File => new FileStream(
