...
public sealed class OtelEventsSamplingProcessor : BaseProcessor<LogRecord>
{
    ...
    /// <summary>
    /// Initializes a new instance of <see cref="OtelEventsSamplingProcessor"/>
    ...
    /// Thrown when <see cref="OtelEventsSamplingOptions.DefaultSamplingRate"/> is outside [0.0, 1.0],
    /// <see cref="OtelEventsSamplingOptions.Strategy"/> is not a defined enum value, or
    /// <see cref="OtelEventsSamplingOptions.ErrorMinLevel"/> is not a defined enum value.
    /// </exception>
    /// <exception cref="ArgumentException">
    /// Thrown when <see cref="OtelEventsSamplingOptions.EventRates"/> contains empty keys,
    /// a bare wildcard <c>"*"</c>, or rate values outside [0.0, 1.0].
    /// </exception>
    public OtelEventsSamplingProcessor(
        OtelEventsSamplingOptions options,
    ...
    internal bool ShouldSample(LogRecord record)
    {
        ...
        if (_options.Strategy == OtelEventsSamplingStrategy.Tail
    ...
    /// Falls back to global <see cref="OtelEventsSamplingOptions.DefaultSamplingRate"/>.
    /// </summary>
    ...
    /// </summary>
    private static void ValidateOptions(OtelEventsSamplingOptions options)
    {
        if (!Enum.IsDefined(options.Strategy))
        {
            throw new ArgumentOutOfRangeException(
                nameof(options),
                options.Strategy,
                "Strategy must be a defined OtelEventsSamplingStrategy enum value.");
        }
